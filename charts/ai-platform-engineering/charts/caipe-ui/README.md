...
## Declarative OAuth connectors and credential secrets

Use `oauthConnectors` to bootstrap OAuth providers into the credential store.
Remote MCP providers may use `mcpUrl` with public PKCE dynamic registration;
generated registration credentials are stored in the encrypted credential
store rather than Helm values.

Use `credentialSecretRefs` for stable, config-driven bearer credentials. Values
are read from environment variables mounted through `existingSecret` or
`externalSecrets` and are encrypted before storage.
